(window.webpackJsonp = window.webpackJsonp || []).push([
	[0],
	{
		10: function(f, v) {},
		11: function(f, v) {},
		2: function(f, v) {},
		3: function(f, v) {},
		4: function(f, v) {},
		"4oAD": function(f, v, o) {
			f.exports = {
				footer: "footer___3ytEA",
				copyright: "copyright___mzWCe",
				"contract-us": "contract-us___Sv-bi",
				links: "links___1JnHZ"
			}
		},
		5: function(f, v) {},
		6: function(f, v) {},
		"61XB": function(f, v) {
			f.exports = "data:image/png;base64,"
		},
		"6JV6": function(f, v, o) {
			f.exports = {
				layout: "layout___3Uxed",
				children: "children___2luU3",
				header: "header___29aXy",
				navBox: "navBox___3AiW-",
				"header-menu": "header-menu___3FURh",
				logo: "logo___NN7QX",
				"menu-outline": "menu-outline___2y7Gd",
				active: "active___3m352",
				accountInfo: "accountInfo___1WUG6"
			}
		},
		7: function(f, v) {},
		8: function(f, v) {},
		9: function(f, v) {},
		Boss: function(f, v, o) {
			"use strict";
			var I = o("2qtc"),
				L = o("kLXV"),
				m = o("14J3"),
				U = o("BMrR"),
				M = o("jCWc"),
				P = o("kPKH"),
				u = o("+L6B"),
				W = o("2/Rp"),
				Q = o("9og8"),
				N = o("WmNS"),
				x = o.n(N),
				D = o("q1tI"),
				Y = o.n(D),
				ee = o("Ydog"),
				E = o("GrFn"),
				j = o.n(E),
				b = o("61XB"),
				J = o.n(b),
				q = o("W5Lp"),
				R = o.n(q),
				re = o("wdOY"),
				Z = o.n(re),
				w = o("r9ys"),
				a = o.n(w),
				e = o("RK7f"),
				t = o.n(e),
				r = o("ej9e"),
				s = o.n(r),
				n = o("nKUr"),
				d = o.n(n),
				c = function(l) {
					var h = l.visible,
						p = l.onCancel,
						A = function(T) {
							T != null && T.address && localStorage.setItem("account-address", T.address)
						},
						g = function() {
							var C = Object(Q.a)(x.a.mark(function T(y) {
								var z;
								return x.a.wrap(function(k) {
									for (;;) switch (k.prev = k.next) {
									case 0:
										return z = new ee.a(y), z.onEnabled(function(K) {
											return A(K)
										}), z.onNetworkChanged(function(K) {
											console.log(K, "onNetworkChanged--------")
										}), z.onAccountChanged(function(K) {
											console.log(K, "onAccountChanged---------")
										}), k.prev = 4, k.next = 7, z.enableBrowserExtension(window.networkEnv);
									case 7:
										k.next = 12;
										break;
									case 9:
										k.prev = 9, k.t0 = k.
										catch (4), console.log(k.t0);
									case 12:
										z.currentAccount && z.currentAccount.address && (console.log("currentAccount--------"), window.dapp = z, localStorage.setItem("connect-method", y), window.location.reload());
									case 13:
									case "end":
										return k.stop()
									}
								}, T, null, [
									[4, 9]
								])
							}));
							return function(y) {
								return C.apply(this, arguments)
							}
						}();
					return Object(n.jsx)(L.a, {
						className: s.a.modal,
						title: "Connect to a wallet",
						visible: h,
						footer: null,
						onCancel: p,
						children: Object(n.jsxs)(U.a, {
							children: [Object(n.jsx)(P.a, {
								span: 24,
								style: {
									textAlign: "center"
								},
								children: Object(n.jsx)(W.a, {
									onClick: function() {
										return g("MetaMask")
									},
									className: s.a.button,
									children: Object(n.jsx)("img", {
										src: j.a,
										alt: "MetaMask"
									})
								})
							}), Object(n.jsx)(P.a, {
								span: 24,
								style: {
									textAlign: "center"
								},
								children: Object(n.jsx)(W.a, {
									onClick: function() {
										return g("WalletConnect")
									},
									className: s.a.button,
									children: Object(n.jsx)("img", {
										src: J.a,
										alt: "WalletConnect"
									})
								})
							}), Object(n.jsx)(P.a, {
								span: 24,
								style: {
									textAlign: "center"
								},
								children: Object(n.jsx)(W.a, {
									onClick: function() {
										return g("TokenPocket")
									},
									className: s.a.button,
									children: Object(n.jsx)("img", {
										src: a.a,
										alt: "TokenPocket"
									})
								})
							}), Object(n.jsx)(P.a, {
								span: 24,
								style: {
									textAlign: "center"
								},
								children: Object(n.jsx)(W.a, {
									onClick: function() {
										return g("MathWallet")
									},
									className: s.a.button,
									children: Object(n.jsx)("img", {
										src: Z.a,
										alt: "MathWallet"
									})
								})
							}), Object(n.jsx)(P.a, {
								span: 24,
								style: {
									textAlign: "center"
								},
								children: Object(n.jsx)(W.a, {
									onClick: function() {
										return g("TrustWallet")
									},
									className: s.a.button,
									children: Object(n.jsx)("img", {
										src: t.a,
										alt: "TrustWallet"
									})
								})
							}), Object(n.jsx)(P.a, {
								span: 24,
								style: {
									textAlign: "center"
								},
								children: Object(n.jsx)(W.a, {
									onClick: function() {
										return g("BinanceWallet")
									},
									className: s.a.button,
									children: Object(n.jsx)("img", {
										src: R.a,
										alt: "BinanceWallet"
									})
								})
							})]
						})
					})
				};
			v.a = c
		},
		GrFn: function(f, v) {
			f.exports = "data:image/png;base64,"
		},
		IpRq: function(f, v, o) {
			"use strict";
			var I = o("k1fw"),
				L = o("tJVT"),
				m = o("q1tI"),
				U = o("9kvl"),
				M = o("b6Qr"),
				P = o("+L6B"),
				u = o("2/Rp"),
				W = o("qVdP"),
				Q = o("jsC+"),
				N = o("5Dmo"),
				x = o("3S7+"),
				D = o("lUTK"),
				Y = o("BvKs"),
				ee = o("tU7J"),
				E = o("wFql"),
				j = o("B9cy"),
				b = o("Ol7k"),
				J = o("Ty5D"),
				q = o("Boss"),
				R = o("mNhR"),
				re = o("DYRE"),
				Z = o("zeV3"),
				w = o("Xsjz"),
				a = o.n(w),
				e = o("nKUr"),
				t = function(B) {
					var G = B.type,
						S = B.link,
						X = B.target,
						V = X === void 0 ? "_blank" : X;
					return Object(e.jsx)(u.a, {
						type: "text",
						href: S,
						children: Object(e.jsx)(R.a, {
							className: a.a.icon,
							type: G,
							target: V
						})
					})
				},
				r = t,
				s = o("SC9r"),
				n = function() {
					return Object(e.jsxs)("div", {
						className: a.a.footer,
						children: [Object(e.jsx)("div", {
							className: a.a.copyright,
							children: "©2021 by RadioCaca.COM"
						}), Object(e.jsx)("div", {
							className: a.a["contract-us"],
							children: "Contact Us：team@radiocaca.com"
						}), Object(e.jsx)("div", {
							className: a.a.links,
							children: Object(e.jsxs)(Z.b, {
								size: "large",
								children: [Object(e.jsx)(r, {
									type: "icon-telegram",
									link: s.c.TelegramGlobal
								}), Object(e.jsx)(r, {
									type: "icon-twitter",
									link: s.c.Twitter
								}), Object(e.jsx)(r, {
									type: "icon-custom1",
									link: s.c.Medium
								})]
							})
						})]
					})
				},
				d = o("4oAD"),
				c = o.n(d),
				i = function() {
					return Object(e.jsxs)("div", {
						className: c.a.footer,
						children: [Object(e.jsx)("div", {
							className: c.a.copyright,
							children: "©2021 by RadioCaca.COM"
						}), Object(e.jsx)("div", {
							className: c.a["contract-us"],
							children: "Contact Us：team@radiocaca.com"
						}), Object(e.jsx)("div", {
							className: c.a.links,
							children: Object(e.jsxs)(Z.b, {
								size: "large",
								children: [Object(e.jsx)(r, {
									type: "icon-telegram",
									link: s.c.TelegramGlobal
								}), Object(e.jsx)(r, {
									type: "icon-twitter",
									link: s.c.Twitter
								}), Object(e.jsx)(r, {
									type: "icon-custom1",
									link: s.c.Medium
								})]
							})
						})]
					})
				},
				l = function() {
					return Object(e.jsx)(e.Fragment, {
						children: M.isMobile ? Object(e.jsx)(i, {}) : Object(e.jsx)(n, {})
					})
				},
				h = o("avyV"),
				p = o.n(h),
				A = function() {
					return Object(e.jsxs)(m.Fragment, {
						children: [M.isMobile && Object(e.jsxs)("div", {
							className: p.a.mobile,
							children: [Object(e.jsx)("img", {
								src: "https://racawebsource.s3-accelerate.amazonaws.com/assets/mainten.png",
								alt: ""
							}), Object(e.jsx)("div", {
								className: p.a.title,
								children: "Server maintenance"
							}), Object(e.jsx)("div", {
								className: p.a.tip,
								children: "Please try again later!"
							})]
						}), !M.isMobile && Object(e.jsxs)("div", {
							className: p.a.pc,
							children: [Object(e.jsx)("img", {
								src: "https://racawebsource.s3-accelerate.amazonaws.com/assets/mainten.png",
								alt: ""
							}), Object(e.jsx)("div", {
								className: p.a.title,
								children: "Server maintenance"
							}), Object(e.jsx)("div", {
								className: p.a.tip,
								children: "Please try again later!"
							})]
						})]
					})
				},
				g = A,
				C = [{
					path: "/drops",
					layout: !0,
					name: "Drops",
					routes: [{
						path: "/drops",
						name: "Drops",
						icon: "smile",
						component: "./Drop"
					}, {
						path: "/drops/:id",
						name: "dropDetail",
						icon: "smile",
						component: "./Drop/Detail"
					}]
				}, {
					path: "/market-place",
					name: "MarketPlace",
					layout: !0,
					routes: [{
						path: "/market-place",
						name: "MarketPlace",
						icon: "smile",
						component: "./MarketPlace"
					}, {
						path: "/market-place/:id",
						name: "marketDetail",
						icon: "smile",
						component: "./MarketPlace/Detail"
					}]
				}, {
					path: "/artwork",
					name: "My Artworks",
					layout: !0,
					routes: [{
						path: "/artwork",
						name: "Artworks",
						icon: "smile",
						component: "./Artwork"
					}, {
						path: "/artwork/:id",
						name: "ArtworksDetail",
						icon: "smile",
						component: "./Artwork/Detail"
					}]
				}, {
					path: "/maintenance",
					name: "maintenance",
					layout: !1,
					icon: "smile",
					component: "./MarketPlace"
				}, {
					path: "/",
					redirect: "/drops"
				}, {
					component: "./404"
				}],
				T = o("Y9ar"),
				y = o.n(T),
				z = b.a.Header,
				F = E.a.Paragraph,
				k = function(B) {
					var G = B.onClickMenu,
						S = B.account,
						X = B.showConnectModal,
						V = B.hideConnectModal,
						_ = B.connectWalletVisible,
						ne = B.accountMenu,
						te = B.children,
						ae = Object(J.g)();
					return Object(e.jsxs)("div", {
						className: y.a.layout,
						children: [Object(e.jsx)(z, {
							className: y.a.header,
							children: Object(e.jsxs)("div", {
								className: y.a.navbox,
								children: [Object(e.jsxs)("span", {
									className: y.a.logo,
									onClick: function() {
										window.open("https://www.radiocaca.com/")
									},
									children: [Object(e.jsx)(R.a, {
										type: "icon-logo",
										style: {
											fontSize: 22
										}
									}), " Radio Caca NFT"]
								}), Object(e.jsx)("div", {
									className: y.a.menuBox,
									children: Object(e.jsx)(Y.a, {
										onClick: G,
										className: y.a["header-menu"],
										mode: "horizontal",
										children: C.map(function(O) {
											return O.layout ? Object(e.jsx)(Y.a.Item, {
												className: "".concat(ae.pathname.includes(O.path) ? y.a.active : ""),
												children: O.name
											}, O.path) : ""
										})
									})
								}), Object(e.jsx)("span", {
									className: y.a.accountInfo,
									children: S ? Object(e.jsx)("span", {
										children: Object(e.jsx)(Q.a, {
											overlay: ne,
											children: Object(e.jsx)("a", {
												className: "ant-dropdown-link",
												onClick: function($) {
													return $.preventDefault()
												},
												children: Object(e.jsx)(x.a, {
													placement: "leftTop",
													title: S,
													children: Object(e.jsx)(F, {
														ellipsis: !0,
														children: S
													})
												})
											})
										})
									}) : Object(e.jsx)(u.a, {
										className: y.a["connect-btn"],
										onClick: function() {
											return X()
										},
										type: "primary",
										children: "Connect Wallet"
									})
								})]
							})
						}), Object(e.jsx)("div", {
							className: y.a.children,
							children: s.a.inMaintenance ? Object(e.jsx)(g, {}) : te
						}), Object(e.jsx)(q.a, {
							visible: _,
							onCancel: function() {
								return V()
							}
						}), Object(e.jsx)(l, {})]
					})
				},
				K = k,
				ce = o("6JV6"),
				H = o.n(ce),
				oe = o("eyie"),
				ue = b.a.Header,
				le = E.a.Paragraph,
				de = function(B) {
					var G = B.onClickMenu,
						S = B.account,
						X = B.showConnectModal,
						V = B.hideConnectModal,
						_ = B.connectWalletVisible,
						ne = B.accountMenu,
						te = B.children,
						ae = Object(J.g)();
					return Object(e.jsxs)("div", {
						className: H.a.layout,
						children: [Object(e.jsx)(ue, {
							className: H.a.header,
							children: Object(e.jsxs)("div", {
								className: H.a.navBox,
								children: [Object(e.jsxs)("div", {
									className: H.a.logo,
									onClick: function() {
										window.open("https://www.radiocaca.com/")
									},
									children: [Object(e.jsx)(R.a, {
										type: "icon-logo",
										style: {
											fontSize: 22
										}
									}), " Radio Caca NFT"]
								}), Object(e.jsx)("span", {
									className: H.a.accountInfo,
									children: S ? Object(e.jsx)("span", {
										children: Object(e.jsx)(Q.a, {
											overlay: ne,
											children: Object(e.jsx)("a", {
												className: "ant-dropdown-link",
												href: "# ",
												onClick: function($) {
													return $.preventDefault()
												},
												children: Object(e.jsx)(x.a, {
													placement: "leftTop",
													title: S,
													children: Object(e.jsx)(le, {
														style: {
															width: 100
														},
														ellipsis: !0,
														children: S
													})
												})
											})
										})
									}) : Object(e.jsx)(u.a, {
										style: {
											width: "100%"
										},
										onClick: function() {
											return X()
										},
										type: "primary",
										children: "Wallet"
									})
								}), Object(e.jsx)("div", {
									className: H.a["menu-outline"],
									children: Object(e.jsx)(Q.a, {
										overlay: Object(e.jsx)(Y.a, {
											onClick: G,
											children: C.map(function(O) {
												return O.layout ? Object(e.jsx)(Y.a.Item, {
													className: "".concat(ae.pathname.includes(O.path) ? H.a.active : ""),
													children: O.name
												}, O.path) : ""
											})
										}),
										trigger: ["click"],
										children: Object(e.jsx)(oe.a, {})
									})
								})]
							})
						}), Object(e.jsx)("div", {
							className: H.a.children,
							children: s.a.inMaintenance ? Object(e.jsx)(g, {}) : te
						}), Object(e.jsx)(q.a, {
							visible: _,
							onCancel: function() {
								return V()
							}
						}), Object(e.jsx)(l, {})]
					})
				},
				he = de,
				pe = function(B) {
					var G = B.children,
						S = Object(m.useState)(!1),
						X = Object(L.a)(S, 2),
						V = X[0],
						_ = X[1],
						ne = function(me) {
							U.a.push(me.key)
						},
						te = function() {
							localStorage.clear(), window.location.reload()
						},
						ae = Object(e.jsx)("div", {
							style: {
								width: "120px",
								height: "40px",
								textAlign: "center",
								lineHeight: "38px",
								background: "#fff",
								color: "#383838",
								border: "1px solid #e5e5e5",
								cursor: "pointer"
							},
							onClick: te,
							children: "Log out"
						}),
						O = localStorage.getItem("account-address"),
						$ = {
							onClickMenu: ne,
							account: O,
							showConnectModal: function() {
								_(!0)
							},
							hideConnectModal: function() {
								_(!1)
							},
							connectWalletVisible: V,
							accountMenu: ae
						};
					return Object(e.jsx)(e.Fragment, {
						children: M.isMobile ? Object(e.jsx)(he, Object(I.a)(Object(I.a)({}, $), {}, {
							children: G
						})) : Object(e.jsx)(K, Object(I.a)(Object(I.a)({}, $), {}, {
							children: G
						}))
					})
				},
				ge = v.a = pe
		},
		RK7f: function(f, v) {
			f.exports = "data:image/png;base64,"
		},
		SC9r: function(f, v, o) {
			"use strict";
			o.d(v, "c", function() {
				return I
			}), o.d(v, "e", function() {
				return P
			}), o.d(v, "d", function() {
				return u
			}), o.d(v, "b", function() {
				return W
			});
			var I = {
				ContractScan: "https://bscscan.com/token/0x12BB890508c125661E03b09EC06E404bc9289040",
				WhereToBuy: "https://www.bakeryswap.org/#/swap?outputCurrency=0x043b49749e0016e965600d502e2177ca2d95b3d9",
				HowToBuy: "https://radiocaca.medium.com/how-to-buy-raca-radio-caca-561b04969442",
				LivePrice: "https://poocoin.app/tokens/0x12bb890508c125661e03b09ec06e404bc9289040",
				TelegramGlobal: "https://t.me/RadioCaca",
				Twitter: "https://twitter.com/RadioCacaNFT",
				Website: "www.radiocaca.com",
				TelegramAsian: "https://t.me/RadioCacaAsian",
				Medium: "https://radiocaca.medium.com/",
				Github: "https://github.com/radiocaca/radio-caca",
				Koda: "https://twitter.com/KodaRobotDog",
				Mxc: "https://www.mexc.com/exchange/RACA_USDT",
				Pancake: "https://www.pancakeswap.finance/swap?outputCurrency=0x12BB890508c125661E03b09EC06E404bc9289040",
				Nft: "https://nft.binance.com/en",
				Tokens: "https://lock.cakecrypt.io/view/0x043b49749e0016e965600d502e2177ca2d95b3d9",
				CERTIK: "https://www.certik.org/projects/radiocaca"
			};

			function L(N) {
				var x = I[N];
				window.open(x)
			}
			var m = {
				RACAV1TokenAddress: "0x48C54f88c07E2EeEbe2895EAC0eCcb6cA18BBe12",
				RACAV2TokenAddress: "0xC8C9b3749BE6F4C1170C3B0a4Ef530Ffa707bB28",
				SwapAddress: "0x71A5fa7ce3A42B8c92e86005858D4bAD478CD10A",
				StakeAddress: "0xa5c1702e2F55C4f9c5cc29FC913363933C8a91cd"
			},
				U = {
					RACAV1TokenAddress: "0x043b49749e0016e965600d502e2177ca2d95b3d9",
					RACAV2TokenAddress: "0x12BB890508c125661E03b09EC06E404bc9289040",
					SwapAddress: "0x7295F9b0bB4e00827EfB4D67212B540443A5c221",
					LPTokenAddress: "0xC016F93D1b11878804c345e93C8588794480CD83",
					StakeAddress: "0x325B2604E3fd2A17C5C99DAb3769051d3BaCa88f"
				};

			function M(N) {
				return N === "main" ? U : m
			}
			function P(N) {
				if (!N) return "0";
				var x = N.toString().split(".");
				return x[0] = x[0].replace(/(\d)(?=(?:\d{3})+$)/g, "$1,"), x.join(".")
			}
			var u = window.fixedPriceSell,
				W = {
					Potion: "https://racawebsource.s3-accelerate.amazonaws.com/assets/1155_img/img_potion.png",
					"Metamon Egg": "https://racawebsource.s3-accelerate.amazonaws.com/assets/1155_img/img_egg.png",
					"Yellow Diamond": "https://racawebsource.s3-accelerate.amazonaws.com/assets/1155_img/img_yellow_diamond.png",
					"Purple Diamond": "https://racawebsource.s3-accelerate.amazonaws.com/assets/1155_img/img_purple_diamond.png",
					"Black Diamond": "https://racawebsource.s3-accelerate.amazonaws.com/assets/1155_img/img_black_diamond.png"
				},
				Q = {
					inMaintenance: !1
				};
			v.a = Q
		},
		W5Lp: function(f, v) {
			f.exports = "data:image/png;base64,"
		},
		Xsjz: function(f, v, o) {
			f.exports = {
				footer: "footer___3EGi_",
				icon: "icon___VjnGp"
			}
		},
		Y9ar: function(f, v, o) {
			f.exports = {
				layout: "layout___1UbRy",
				children: "children___fLnSE",
				header: "header___31FlK",
				navbox: "navbox___2dNew",
				menuBox: "menuBox___1Z0GM",
				"header-menu": "header-menu___3LXMQ",
				active: "active___2ot4W",
				accountInfo: "accountInfo___14KiY",
				logo: "logo___2z_4F",
				"connect-btn": "connect-btn___2Jyu3"
			}
		},
		YLtA: function(f, v, o) {
			"use strict";
			o.d(v, "a", function() {
				return u
			});
			var I = o("9og8"),
				L = o("WmNS"),
				m = o.n(L),
				U = o("Ydog"),
				M = o("tTNn"),
				P = o.t("tTNn", 1),
				u = function() {
					var W = Object(I.a)(m.a.mark(function Q() {
						var N, x, D, Y;
						return m.a.wrap(function(E) {
							for (;;) switch (E.prev = E.next) {
							case 0:
								if (!(window.dapp || !localStorage.getItem("account-address"))) {
									E.next = 2;
									break
								}
								return E.abrupt("return");
							case 2:
								return N = function(b) {
									b != null && b.address && localStorage.setItem("account-address", b.address)
								}, x = localStorage.getItem("connect-method"), D = new U.a(x), D.onEnabled(function(j) {
									return N(j)
								}), E.prev = 6, E.next = 9, D.enableBrowserExtension(window.networkEnv);
							case 9:
								E.next = 14;
								break;
							case 11:
								E.prev = 11, E.t0 = E.
								catch (6), console.log(E.t0);
							case 14:
								if (!(D.currentAccount && D.currentAccount.address)) {
									E.next = 22;
									break
								}
								return window.dapp = D, D.onNetworkChanged(function(j) {
									console.log(j, "onNetworkChanged--------")
								}), D.onAccountChanged(function(j) {
									console.log(j, "onAccountChanged---------"), j != null && j.address && (localStorage.setItem("account-address", j.address), window.location.reload())
								}), E.next = 20, D.getBalance(D.currentAccount.address, window.racaAddress, M, 18);
							case 20:
								Y = E.sent, window.balance = Y;
							case 22:
							case "end":
								return E.stop()
							}
						}, Q, null, [
							[6, 11]
						])
					}));
					return function() {
						return W.apply(this, arguments)
					}
				}()
		},
		Ydog: function(f, v, o) {
			"use strict";
			o.d(v, "a", function() {
				return Z
			});
			var I = o("k1fw"),
				L = o("PpiC"),
				m = o("9og8"),
				U = o("fWQN"),
				M = o("mtLc"),
				P = o("WmNS"),
				u = o.n(P),
				W = [{
					id: "test",
					name: "BSC Testnet",
					chainId: "0x61",
					url: "https://data-seed-prebsc-2-s3.binance.org:8545/",
					explorer: "https://testnet.bscscan.com/",
					config: {
						chainId: "0x61",
						chainName: "BSC Testnet",
						nativeCurrency: {
							name: "BNB",
							symbol: "BNB",
							decimals: 18
						},
						rpcUrls: ["https://bsc.getblock.io/testnet/?api_key=58f594f0-cc9e-4db2-a87a-009ec7790563"],
						blockExplorerUrls: ["https://testnet.bscscan.com/"]
					}
				}, {
					id: "main",
					name: "BSC Mainnet",
					chainId: "0x38",
					url: "https://bsc-dataseed.binance.org/",
					explorer: "https://bscscan.com/",
					config: {
						chainId: "0x38",
						chainName: "Binance Smart Chain",
						nativeCurrency: {
							name: "BNB",
							symbol: "BNB",
							decimals: 18
						},
						rpcUrls: ["https://bsc-dataseed.binance.org/"],
						blockExplorerUrls: ["https://bscscan.com/"]
					}
				}],
				Q = W,
				N = o("/xke"),
				x = o("TeRw"),
				D = function() {
					function w() {
						if (Object(U.a)(this, w), !window.BinanceChain) {
							x.
						default.warning({
								message: "BSC Connect Failed",
								description: "Provider error, no provider was found"
							});
							return
						}
						this.name = "BinanceWallet", this._accounts = [], this._enabled = !1, this.bcWallet = window.BinanceChain, this._chainId = void 0, this._chainId = void 0, this._onEnabled = void 0, this._currentAccount = void 0
					}
					return Object(M.a)(w, [{
						key: "isEnabled",
						get: function() {
							return this._enabled
						}
					}, {
						key: "enable",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t() {
								var r = this,
									s, n, d = arguments;
								return u.a.wrap(function(i) {
									for (;;) switch (i.prev = i.next) {
									case 0:
										return s = d.length > 0 && d[0] !== void 0 ? d[0] : "main", i.next = 3, this.bcWallet.request({
											method: "eth_requestAccounts"
										});
									case 3:
										return n = i.sent, this._currentAccount = {
											address: n[0]
										}, i.next = 7, this.bcWallet.request({
											method: "eth_chainId"
										});
									case 7:
										return this._chainId = i.sent, this.bcWallet.on("chainChanged", function(l) {
											r._chainId = l, r._onNetworkChanged && r._onNetworkChanged(r.getNetwork())
										}), this.bcWallet.on("accountsChanged", function(l) {
											r._currentAccount = {
												address: l[0]
											}, r._onAccountChanged && r._onAccountChanged({
												address: l[0]
											})
										}), this._enabled = !0, this._onEnabled && this._onEnabled({
											address: n[0]
										}), i.abrupt("return", this._chainId);
									case 13:
									case "end":
										return i.stop()
									}
								}, t, this)
							}));

							function e() {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "dispose",
						value: function() {}
					}, {
						key: "onEnabled",
						value: function(e) {
							var t = this;
							return this._onEnabled = e, function() {
								return t._onEnabled = void 0
							}
						}
					}, {
						key: "chainId",
						get: function() {
							return this._chainId
						}
					}, {
						key: "getNetwork",
						value: function() {
							var e = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : this.chainId;
							return {
								chainId: e,
								isBscMainnet: e === "0x38",
								isBscTestnet: e === "0x61"
							}
						}
					}, {
						key: "onNetworkChanged",
						value: function(e) {
							var t = this;
							return this._onNetworkChanged = e, function() {
								return t._onNetworkChanged = void 0
							}
						}
					}, {
						key: "currentAccount",
						get: function() {
							return this._currentAccount
						}
					}, {
						key: "getAllAccounts",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t() {
								return u.a.wrap(function(s) {
									for (;;) switch (s.prev = s.next) {
									case 0:
										return s.next = 2, this.bcWallet.requestAccounts();
									case 2:
										return this._accounts = s.sent.map(function(n) {
											return n.address = n.addresses.find(function(d) {
												return d.type === "eth"
											}).address, n
										}), s.abrupt("return", this._accounts);
									case 4:
									case "end":
										return s.stop()
									}
								}, t, this)
							}));

							function e() {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "onAccountChanged",
						value: function(e) {
							var t = this;
							return this._onAccountChanged = e, function() {
								return t._onAccountChanged = void 0
							}
						}
					}, {
						key: "signMessage",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r) {
								return u.a.wrap(function(n) {
									for (;;) switch (n.prev = n.next) {
									case 0:
										return n.abrupt("return", this.bcWallet.request({
											method: "eth_sign",
											params: [this.currentAccount.address, r]
										}));
									case 1:
									case "end":
										return n.stop()
									}
								}, t, this)
							}));

							function e(t) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "signTypedData",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r) {
								return u.a.wrap(function(n) {
									for (;;) switch (n.prev = n.next) {
									case 0:
										throw new Error("signTypedData is not supported for Binance Chain Wallet.");
									case 1:
									case "end":
										return n.stop()
									}
								}, t)
							}));

							function e(t) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "sendTransaction",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r) {
								return u.a.wrap(function(n) {
									for (;;) switch (n.prev = n.next) {
									case 0:
										return n.abrupt("return", this.bcWallet.request({
											method: "eth_sendTransaction",
											params: [r]
										}));
									case 1:
									case "end":
										return n.stop()
									}
								}, t, this)
							}));

							function e(t) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}]), w
				}(),
				Y = function() {
					function w(a) {
						Object(U.a)(this, w), this.name = a != null ? a : "Metamask", this._accounts = [], this._enabled = !1, this.ethereum = window.ethereum, this._chainId = void 0, this._onEnabled = void 0, this._currentAccount = void 0
					}
					return Object(M.a)(w, [{
						key: "isEnabled",
						get: function() {
							return this._enabled
						}
					}, {
						key: "enable",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t() {
								var r = this,
									s, n, d, c = arguments;
								return u.a.wrap(function(l) {
									for (;;) switch (l.prev = l.next) {
									case 0:
										return s = c.length > 0 && c[0] !== void 0 ? c[0] : "main", l.next = 3, this.ethereum.request({
											method: "eth_requestAccounts"
										});
									case 3:
										return n = l.sent, this._currentAccount = {
											address: n[0]
										}, d = Q.find(function(h) {
											return h.id === s
										}), l.next = 8, this.ethereum.request({
											method: "wallet_addEthereumChain",
											params: [d == null ? void 0 : d.config]
										});
									case 8:
										return l.next = 10, this.ethereum.request({
											method: "eth_chainId"
										});
									case 10:
										return this._chainId = l.sent, this.ethereum.on("chainChanged", function(h) {
											r._chainId = h, r._onNetworkChanged && r._onNetworkChanged(r.getNetwork())
										}), this.ethereum.on("accountsChanged", function(h) {
											r._currentAccount = {
												address: h[0]
											}, r._onAccountChanged && r._onAccountChanged({
												address: h[0]
											})
										}), this._enabled = !0, this._onEnabled && this._onEnabled({
											address: n[0]
										}), l.abrupt("return", this._chainId);
									case 16:
									case "end":
										return l.stop()
									}
								}, t, this)
							}));

							function e() {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "dispose",
						value: function() {
							this.ethereum.removeAllListeners("chainChanged"), this.ethereum.removeAllListeners("accountsChanged")
						}
					}, {
						key: "onEnabled",
						value: function(e) {
							var t = this;
							return this._onEnabled = e, function() {
								return t._onEnabled = void 0
							}
						}
					}, {
						key: "chainId",
						get: function() {
							return this._chainId
						}
					}, {
						key: "getNetwork",
						value: function() {
							var e = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : this.chainId;
							return {
								chainId: e,
								isBscMainnet: e === "0x38",
								isBscTestnet: e === "0x61"
							}
						}
					}, {
						key: "onNetworkChanged",
						value: function(e) {
							var t = this;
							return this._onNetworkChanged = e, function() {
								return t._onNetworkChanged = void 0
							}
						}
					}, {
						key: "currentAccount",
						get: function() {
							return this._currentAccount
						}
					}, {
						key: "getAllAccounts",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t() {
								var r, s;
								return u.a.wrap(function(d) {
									for (;;) switch (d.prev = d.next) {
									case 0:
										return d.next = 2, this.ethereum.request({
											method: "wallet_getPermissions"
										});
									case 2:
										return r = d.sent, s = r[0].caveats.find(function(c) {
											return c.type === "filterResponse"
										}), this._accounts = (s ? s.value : []).map(function(c) {
											return {
												address: c
											}
										}), d.abrupt("return", this._accounts);
									case 6:
									case "end":
										return d.stop()
									}
								}, t, this)
							}));

							function e() {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "onAccountChanged",
						value: function(e) {
							var t = this;
							return this._onAccountChanged = e, function() {
								return t._onAccountChanged = void 0
							}
						}
					}, {
						key: "signMessage",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r) {
								return u.a.wrap(function(n) {
									for (;;) switch (n.prev = n.next) {
									case 0:
										return n.abrupt("return", this.ethereum.request({
											method: "eth_sign",
											params: [this.currentAccount.address, r]
										}));
									case 1:
									case "end":
										return n.stop()
									}
								}, t, this)
							}));

							function e(t) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "signTypedData",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r) {
								return u.a.wrap(function(n) {
									for (;;) switch (n.prev = n.next) {
									case 0:
										return n.abrupt("return", this.ethereum.request({
											method: "eth_signTypedData",
											params: [r, this.currentAccount.address],
											from: this.currentAccount.address
										}));
									case 1:
									case "end":
										return n.stop()
									}
								}, t, this)
							}));

							function e(t) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "sendTransaction",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r) {
								return u.a.wrap(function(n) {
									for (;;) switch (n.prev = n.next) {
									case 0:
										return n.abrupt("return", this.ethereum.request({
											method: "eth_sendTransaction",
											params: [r]
										}));
									case 1:
									case "end":
										return n.stop()
									}
								}, t, this)
							}));

							function e(t) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}]), w
				}(),
				ee = o("Lq9n"),
				E = function() {
					function w(a) {
						Object(U.a)(this, w), this.name = "WalletConnect", this._accounts = [], this._enabled = !1, this.connector = new ee.a(a), this._chainId = void 0, this._onEnabled = void 0, this._currentAccount = void 0
					}
					return Object(M.a)(w, [{
						key: "enable",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t() {
								var r = this,
									s, n, d = arguments;
								return u.a.wrap(function(i) {
									for (;;) switch (i.prev = i.next) {
									case 0:
										return s = d.length > 0 && d[0] !== void 0 ? d[0] : "main", i.next = 3, this.connector.enable();
									case 3:
										return n = this.connector.accounts, this._currentAccount = {
											address: n[0]
										}, this._chainId = "0x".concat(this.connector.chainId.toString(16)), this.connector.on("accountsChanged", function(l) {
											r._currentAccount = {
												address: l[0]
											}, r._onAccountChanged && r._onAccountChanged({
												address: l[0]
											})
										}), this.connector.on("chainChanged", function(l) {
											console.log(l), r._chainId = l, r._onNetworkChanged && r._onNetworkChanged(r.getNetwork())
										}), this.connector.on("disconnect", function() {
											var l = Object(m.a)(u.a.mark(function h(p, A) {
												return u.a.wrap(function(C) {
													for (;;) switch (C.prev = C.next) {
													case 0:
														r._onDisconnect && r._onDisconnect(), localStorage.clear(), window.location.reload();
													case 3:
													case "end":
														return C.stop()
													}
												}, h)
											}));
											return function(h, p) {
												return l.apply(this, arguments)
											}
										}()), this._enabled = !0, this._onEnabled && this._onEnabled({
											address: n[0]
										}), i.abrupt("return", this._chainId);
									case 12:
									case "end":
										return i.stop()
									}
								}, t, this)
							}));

							function e() {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "onEnabled",
						value: function(e) {
							var t = this;
							return this._onEnabled = e, function() {
								return t._onEnabled = void 0
							}
						}
					}, {
						key: "chainId",
						get: function() {
							return this._chainId
						}
					}, {
						key: "getNetwork",
						value: function() {
							var e = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : this.chainId;
							return {
								chainId: e,
								isBscMainnet: e === "0x38",
								isBscTestnet: e === "0x61"
							}
						}
					}, {
						key: "onNetworkChanged",
						value: function(e) {
							var t = this;
							return this._onNetworkChanged = e, function() {
								return t._onNetworkChanged = void 0
							}
						}
					}, {
						key: "currentAccount",
						get: function() {
							return this._currentAccount
						}
					}, {
						key: "onAccountChanged",
						value: function(e) {
							var t = this;
							return this._onAccountChanged = e, e(), function() {
								return t._onAccountChanged = void 0
							}
						}
					}, {
						key: "onDisconnect",
						value: function(e) {
							var t = this;
							return this._onDisconnect = e, function() {
								return t._onDisconnect = void 0
							}
						}
					}, {
						key: "sendTransaction",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r) {
								return u.a.wrap(function(n) {
									for (;;) switch (n.prev = n.next) {
									case 0:
										return n.abrupt("return", this.connector.request({
											method: "eth_sendTransaction",
											params: [r]
										}));
									case 1:
									case "end":
										return n.stop()
									}
								}, t, this)
							}));

							function e(t) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}]), w
				}(),
				j = o("oBTY"),
				b = o("wDBh"),
				J = o("Qyje"),
				q = o.n(J),
				R = function() {
					function w(a) {
						var e = this,
							t = a.url,
							r = a.explorer;
						Object(U.a)(this, w), this.getBalance = function() {
							var s = Object(m.a)(u.a.mark(function n(d, c, i, l) {
								var h, p;
								return u.a.wrap(function(g) {
									for (;;) switch (g.prev = g.next) {
									case 0:
										return h = null, p = new b.a.Contract(c, i, e.provider), g.next = 4, p.functions.balanceOf(d).then(function() {
											var C = Object(m.a)(u.a.mark(function T(y) {
												return u.a.wrap(function(F) {
													for (;;) switch (F.prev = F.next) {
													case 0:
														h = b.a.utils.formatUnits(y.toString(), l);
													case 1:
													case "end":
														return F.stop()
													}
												}, T)
											}));
											return function(T) {
												return C.apply(this, arguments)
											}
										}());
									case 4:
										return g.abrupt("return", h);
									case 5:
									case "end":
										return g.stop()
									}
								}, n)
							}));
							return function(n, d, c, i) {
								return s.apply(this, arguments)
							}
						}(), this.getName = function() {
							var s = Object(m.a)(u.a.mark(function n(d, c) {
								var i, l;
								return u.a.wrap(function(p) {
									for (;;) switch (p.prev = p.next) {
									case 0:
										return i = null, l = new b.a.Contract(d, c, e.provider), p.next = 4, l.functions.name().then(function() {
											var A = Object(m.a)(u.a.mark(function g(C) {
												return u.a.wrap(function(y) {
													for (;;) switch (y.prev = y.next) {
													case 0:
														i = C.toString();
													case 1:
													case "end":
														return y.stop()
													}
												}, g)
											}));
											return function(g) {
												return A.apply(this, arguments)
											}
										}());
									case 4:
										return p.abrupt("return", i);
									case 5:
									case "end":
										return p.stop()
									}
								}, n)
							}));
							return function(n, d) {
								return s.apply(this, arguments)
							}
						}(), this.getAllowance = function() {
							var s = Object(m.a)(u.a.mark(function n(d, c, i, l, h) {
								var p, A;
								return u.a.wrap(function(C) {
									for (;;) switch (C.prev = C.next) {
									case 0:
										return console.log("getAllowance", d, c, i, l, h), p = null, A = new b.a.Contract(c, l, e.provider), C.next = 5, A.functions.allowance(d, i).then(function() {
											var T = Object(m.a)(u.a.mark(function y(z) {
												return u.a.wrap(function(k) {
													for (;;) switch (k.prev = k.next) {
													case 0:
														p = b.a.utils.formatUnits(z.toString(), h), console.log("allowance=", p);
													case 2:
													case "end":
														return k.stop()
													}
												}, y)
											}));
											return function(y) {
												return T.apply(this, arguments)
											}
										}());
									case 5:
										return C.abrupt("return", p);
									case 6:
									case "end":
										return C.stop()
									}
								}, n)
							}));
							return function(n, d, c, i, l) {
								return s.apply(this, arguments)
							}
						}(), this.provider = new b.a.providers.Web3Provider(window.ethereum), this.explorer = new re(r)
					}
					return Object(M.a)(w, [{
						key: "getAccount",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r) {
								var s, n, d;
								return u.a.wrap(function(i) {
									for (;;) switch (i.prev = i.next) {
									case 0:
										return i.next = 2, this.provider.getBalance(r);
									case 2:
										return s = i.sent, i.next = 5, this.provider.getCode(r);
									case 5:
										return n = i.sent, d = b.a.utils.keccak256(n), i.abrupt("return", {
											balance: s,
											codeHash: d
										});
									case 8:
									case "end":
										return i.stop()
									}
								}, t, this)
							}));

							function e(t) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "executeContract",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s, n) {
								var d, c, i, l, h = arguments;
								return u.a.wrap(function(A) {
									for (;;) switch (A.prev = A.next) {
									case 0:
										return d = h.length > 3 && h[3] !== void 0 ? h[3] : [], c = h.length > 4 && h[4] !== void 0 ? h[4] : {}, i = new b.a.Contract(r, s, this.provider.getSigner()), A.next = 5, i[n].apply(i, Object(j.a)(d).concat([c]));
									case 5:
										return l = A.sent, A.abrupt("return", l);
									case 7:
									case "end":
										return A.stop()
									}
								}, t, this)
							}));

							function e(t, r, s) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "getEstimation",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s) {
								var n, d, c, i, l, h, p = arguments;
								return u.a.wrap(function(g) {
									for (;;) switch (g.prev = g.next) {
									case 0:
										return d = p.length > 2 && p[2] !== void 0 ? p[2] : "buy", c = p.length > 3 && p[3] !== void 0 ? p[3] : [], i = p.length > 4 && p[4] !== void 0 ? p[4] : {}, l = new b.a.Contract(r, s, this.provider.getSigner()), g.next = 6, (n = l.estimateGas).buy.apply(n, Object(j.a)(c).concat([i]));
									case 6:
										return h = g.sent, g.abrupt("return", h);
									case 8:
									case "end":
										return g.stop()
									}
								}, t, this)
							}));

							function e(t, r) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "buyByBnb",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s) {
								var n, d, c, i, l, h, p = arguments;
								return u.a.wrap(function(g) {
									for (;;) switch (g.prev = g.next) {
									case 0:
										return n = p.length > 2 && p[2] !== void 0 ? p[2] : "buy", d = p.length > 3 && p[3] !== void 0 ? p[3] : [], c = p.length > 4 && p[4] !== void 0 ? p[4] : {}, g.next = 5, this.getEstimation(r, s, n, d, c);
									case 5:
										return i = g.sent, c.gasLimit = (i.toNumber() * 3 / 2).toFixed(0), l = new b.a.Contract(r, s, this.provider.getSigner()), g.next = 10, l[n].apply(l, Object(j.a)(d).concat([c]));
									case 10:
										return h = g.sent, g.abrupt("return", h);
									case 12:
									case "end":
										return g.stop()
									}
								}, t, this)
							}));

							function e(t, r) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "runContractTransactionFunc",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s, n) {
								var d, c, i, l, h, p = arguments;
								return u.a.wrap(function(g) {
									for (;;) switch (g.prev = g.next) {
									case 0:
										for (d = new b.a.Contract(r, s, this.provider.getSigner()), c = p.length, i = new Array(c > 3 ? c - 3 : 0), l = 3; l < c; l++) i[l - 3] = p[l];
										return g.next = 4, d[n].apply(d, i);
									case 4:
										return h = g.sent, g.abrupt("return", h);
									case 6:
									case "end":
										return g.stop()
									}
								}, t, this)
							}));

							function e(t, r, s) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "queryContract",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s, n) {
								var d, c, i, l, h, p = arguments;
								return u.a.wrap(function(g) {
									for (;;) switch (g.prev = g.next) {
									case 0:
										for (c = new b.a.Contract(r, s, this.provider), i = p.length, l = new Array(i > 3 ? i - 3 : 0), h = 3; h < i; h++) l[h - 3] = p[h];
										return g.abrupt("return", (d = c.functions)[n].apply(d, l));
									case 3:
									case "end":
										return g.stop()
									}
								}, t, this)
							}));

							function e(t, r, s) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "approve",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s, n, d) {
								var c, i;
								return u.a.wrap(function(h) {
									for (;;) switch (h.prev = h.next) {
									case 0:
										return c = new b.a.Contract(r, s, this.provider.getSigner()), i = c.functions.approve(n, d), h.abrupt("return", i);
									case 3:
									case "end":
										return h.stop()
									}
								}, t, this)
							}));

							function e(t, r, s, n) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "swap",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s, n) {
								var d, c;
								return u.a.wrap(function(l) {
									for (;;) switch (l.prev = l.next) {
									case 0:
										return d = new b.a.Contract(r, s, this.provider.getSigner()), c = d.functions.swap(n), l.abrupt("return", c);
									case 3:
									case "end":
										return l.stop()
									}
								}, t, this)
							}));

							function e(t, r, s) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "stake",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s, n) {
								var d, c;
								return u.a.wrap(function(l) {
									for (;;) switch (l.prev = l.next) {
									case 0:
										return d = new b.a.Contract(r, s, this.provider.getSigner()), c = d.functions.stake(n), l.abrupt("return", c);
									case 3:
									case "end":
										return l.stop()
									}
								}, t, this)
							}));

							function e(t, r, s) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "harvest",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s, n, d) {
								var c, i;
								return u.a.wrap(function(h) {
									for (;;) switch (h.prev = h.next) {
									case 0:
										return c = new b.a.Contract(r, s, this.provider.getSigner()), i = c.functions.harvest(n, d), h.abrupt("return", i);
									case 3:
									case "end":
										return h.stop()
									}
								}, t, this)
							}));

							function e(t, r, s, n) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "forceWithdraw",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s, n, d) {
								var c, i;
								return u.a.wrap(function(h) {
									for (;;) switch (h.prev = h.next) {
									case 0:
										return c = new b.a.Contract(r, s, this.provider.getSigner()), i = c.functions.forceWithdraw(n, d), h.abrupt("return", i);
									case 3:
									case "end":
										return h.stop()
									}
								}, t, this)
							}));

							function e(t, r, s, n) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "getTransactions",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s, n) {
								var d;
								return u.a.wrap(function(i) {
									for (;;) switch (i.prev = i.next) {
									case 0:
										return i.next = 2, this.explorer.getHistory(r, s, n);
									case 2:
										return d = i.sent, i.abrupt("return", {
											length: 0,
											list: d.result
										});
									case 4:
									case "end":
										return i.stop()
									}
								}, t, this)
							}));

							function e(t, r, s) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "parseUnits",
						value: function(e, t) {
							return b.a.utils.parseUnits(e, t)
						}
					}, {
						key: "formatUnits",
						value: function(e, t) {
							return b.a.utils.formatUnits(e, t)
						}
					}, {
						key: "parseEther",
						value: function(e) {
							return b.a.utils.parseEther(e)
						}
					}]), w
				}(),
				re = function() {
					function w(a) {
						Object(U.a)(this, w), this.url = a
					}
					return Object(M.a)(w, [{
						key: "getHistory",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r) {
								var s, n, d, c, i, l = arguments;
								return u.a.wrap(function(p) {
									for (;;) switch (p.prev = p.next) {
									case 0:
										return s = l.length > 1 && l[1] !== void 0 ? l[1] : 0, n = l.length > 2 && l[2] !== void 0 ? l[2] : 10, d = {
											module: "account",
											action: "txlist",
											address: r,
											startblock: 0,
											endblock: 99999999,
											page: s + 1,
											offset: n,
											sort: "desc"
										}, p.next = 5, fetch("".concat(this.url, "?").concat(q.a.stringify(d)));
									case 5:
										return c = p.sent, p.next = 8, c.json();
									case 8:
										return i = p.sent, p.abrupt("return", i);
									case 10:
									case "end":
										return p.stop()
									}
								}, t, this)
							}));

							function e(t) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}]), w
				}(),
				Z = function() {
					function w(a) {
						var e = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : Q;
						Object(U.a)(this, w), this.id = new Date().valueOf(), console.log(this.id, "dapp-id", a, e), this._client = null, this._networks = e;
						try {
							a === "BinanceWallet" ? this._browserExtension = new D : a === "WalletConnect" ? (this._browserExtension = new E({
								rpc: {
									97: "https://data-seed-prebsc-2-s3.binance.org:8545/",
									56: "https://bsc-dataseed.binance.org/"
								},
								chainId: window.networkEnv === "main" ? 56 : 97
							}), window.ethereum = this._browserExtension.connector) : this._browserExtension = new Y(a)
						} catch (t) {
							console.warn("Unable to init the dApp. No compatible browser extension is found."), console.error("connecting error: ", t.toString())
						}
					}
					return Object(M.a)(w, [{
						key: "queryContract",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s, n) {
								var d, c, i, l, h = arguments;
								return u.a.wrap(function(A) {
									for (;;) switch (A.prev = A.next) {
									case 0:
										for (c = h.length, i = new Array(c > 3 ? c - 3 : 0), l = 3; l < c; l++) i[l - 3] = h[l];
										return A.abrupt("return", (d = this._client).queryContract.apply(d, [r, s, n].concat(i)));
									case 2:
									case "end":
										return A.stop()
									}
								}, t, this)
							}));

							function e(t, r, s) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "runContractTransactionFunc",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s, n) {
								var d, c, i, l, h = arguments;
								return u.a.wrap(function(A) {
									for (;;) switch (A.prev = A.next) {
									case 0:
										for (c = h.length, i = new Array(c > 3 ? c - 3 : 0), l = 3; l < c; l++) i[l - 3] = h[l];
										return A.abrupt("return", (d = this._client).runContractTransactionFunc.apply(d, [r, s, n].concat(i)));
									case 2:
									case "end":
										return A.stop()
									}
								}, t, this)
							}));

							function e(t, r, s) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "browserExtension",
						get: function() {
							return this._browserExtension
						}
					}, {
						key: "isBrowserExtensionInstalled",
						get: function() {
							return Boolean(this.browserExtension)
						}
					}, {
						key: "isBrowserExtensionEnabled",
						get: function() {
							return this.isBrowserExtensionInstalled && this.browserExtension.isEnabled
						}
					}, {
						key: "enableBrowserExtension",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t() {
								var r, s, n = arguments;
								return u.a.wrap(function(c) {
									for (;;) switch (c.prev = c.next) {
									case 0:
										if (r = n.length > 0 && n[0] !== void 0 ? n[0] : "test", s = "", !(this.isBrowserExtensionInstalled || localStorage.getItem("walletconnect"))) {
											c.next = 6;
											break
										}
										return c.next = 5, this.browserExtension.enable(r);
									case 5:
										s = c.sent;
									case 6:
										return this.initRpcFromChainId(s), c.abrupt("return", s);
									case 8:
									case "end":
										return c.stop()
									}
								}, t, this)
							}));

							function e() {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "onEnabled",
						value: function(e) {
							return this.isBrowserExtensionInstalled && this.browserExtension.onEnabled(e)
						}
					}, {
						key: "network",
						get: function() {
							return this.isBrowserExtensionInstalled && this.browserExtension.getNetwork()
						}
					}, {
						key: "onNetworkChanged",
						value: function(e) {
							var t = this,
								r = function(n) {
									t.initRpcFromChainId(n.chainId), e(n)
								};
							return this.isBrowserExtensionInstalled && this.browserExtension.onNetworkChanged(r)
						}
					}, {
						key: "initRpcFromChainId",
						value: function(e) {
							if (e) {
								var t = this._networks.find(function(r) {
									return r.chainId === e
								});
								t && (this._client = new R(t))
							}
						}
					}, {
						key: "rpc",
						get: function() {
							return this._client && this._client.provider
						}
					}, {
						key: "explorer",
						get: function() {
							return this._client && this._client.explorer
						}
					}, {
						key: "currentAccount",
						get: function() {
							return this.isBrowserExtensionInstalled && this.browserExtension.currentAccount
						}
					}, {
						key: "onAccountChanged",
						value: function(e) {
							return this.isBrowserExtensionInstalled && this.browserExtension.onAccountChanged(e)
						}
					}, {
						key: "onDisconnect",
						value: function(e) {
							return this.isBrowserExtensionInstalled && this.browserExtension.onDisconnect(e)
						}
					}, {
						key: "getAllAccounts",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t() {
								return u.a.wrap(function(s) {
									for (;;) switch (s.prev = s.next) {
									case 0:
										return s.abrupt("return", this.isBrowserExtensionInstalled && this.browserExtension.getAllAccounts());
									case 1:
									case "end":
										return s.stop()
									}
								}, t, this)
							}));

							function e() {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "signMessage",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r) {
								return u.a.wrap(function(n) {
									for (;;) switch (n.prev = n.next) {
									case 0:
										return n.abrupt("return", this.isBrowserExtensionInstalled && this.browserExtension.signMessage(r));
									case 1:
									case "end":
										return n.stop()
									}
								}, t, this)
							}));

							function e(t) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "signTypedData",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r) {
								return u.a.wrap(function(n) {
									for (;;) switch (n.prev = n.next) {
									case 0:
										return n.abrupt("return", this.isBrowserExtensionInstalled && this.browserExtension.signTypedData(r));
									case 1:
									case "end":
										return n.stop()
									}
								}, t, this)
							}));

							function e(t) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "sendTransaction",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r) {
								var s, n, d, c;
								return u.a.wrap(function(l) {
									for (;;) switch (l.prev = l.next) {
									case 0:
										return s = r.from, n = r.to, d = r.value, c = Object(L.a)(r, ["from", "to", "value"]), l.abrupt("return", this.isBrowserExtensionInstalled && this.browserExtension.sendTransaction(Object(I.a)({
											from: s,
											to: n,
											value: d.toHexString()
										}, c)));
									case 2:
									case "end":
										return l.stop()
									}
								}, t, this)
							}));

							function e(t) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "executeContract",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s, n) {
								var d, c, i = arguments;
								return u.a.wrap(function(h) {
									for (;;) switch (h.prev = h.next) {
									case 0:
										return d = i.length > 3 && i[3] !== void 0 ? i[3] : [], c = i.length > 4 && i[4] !== void 0 ? i[4] : {}, h.abrupt("return", this._client.executeContract(r, s, n, d, c));
									case 3:
									case "end":
										return h.stop()
									}
								}, t, this)
							}));

							function e(t, r, s) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "buyByBnb",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s, n) {
								var d, c, i = arguments;
								return u.a.wrap(function(h) {
									for (;;) switch (h.prev = h.next) {
									case 0:
										return d = i.length > 3 && i[3] !== void 0 ? i[3] : [], c = i.length > 4 && i[4] !== void 0 ? i[4] : {}, h.abrupt("return", this._client.buyByBnb(r, s, n, d, c));
									case 3:
									case "end":
										return h.stop()
									}
								}, t, this)
							}));

							function e(t, r, s) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "approve",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s, n, d) {
								return u.a.wrap(function(i) {
									for (;;) switch (i.prev = i.next) {
									case 0:
										return i.abrupt("return", this._client.approve(r, s, n, d));
									case 1:
									case "end":
										return i.stop()
									}
								}, t, this)
							}));

							function e(t, r, s, n) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "swap",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s, n) {
								return u.a.wrap(function(c) {
									for (;;) switch (c.prev = c.next) {
									case 0:
										return c.abrupt("return", this._client.swap(r, s, n));
									case 1:
									case "end":
										return c.stop()
									}
								}, t, this)
							}));

							function e(t, r, s) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "stake",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s, n) {
								return u.a.wrap(function(c) {
									for (;;) switch (c.prev = c.next) {
									case 0:
										return c.abrupt("return", this._client.stake(r, s, n));
									case 1:
									case "end":
										return c.stop()
									}
								}, t, this)
							}));

							function e(t, r, s) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "harvest",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s, n, d) {
								return u.a.wrap(function(i) {
									for (;;) switch (i.prev = i.next) {
									case 0:
										return i.abrupt("return", this._client.harvest(r, s, n, d));
									case 1:
									case "end":
										return i.stop()
									}
								}, t, this)
							}));

							function e(t, r, s, n) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "forceWithdraw",
						value: function() {
							var a = Object(m.a)(u.a.mark(function t(r, s, n, d) {
								return u.a.wrap(function(i) {
									for (;;) switch (i.prev = i.next) {
									case 0:
										return i.abrupt("return", this._client.forceWithdraw(r, s, n, d));
									case 1:
									case "end":
										return i.stop()
									}
								}, t, this)
							}));

							function e(t, r, s, n) {
								return a.apply(this, arguments)
							}
							return e
						}()
					}, {
						key: "getAllowance",
						value: function(e, t, r, s) {
							var n = arguments.length > 4 && arguments[4] !== void 0 ? arguments[4] : 18;
							return this._client.getAllowance(e, t, r, s, n)
						}
					}, {
						key: "getBalance",
						value: function(e, t, r) {
							var s = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : 18;
							return this._client.getBalance(e, t, r, s)
						}
					}, {
						key: "getName",
						value: function(e, t) {
							return this._client.getName(e, t)
						}
					}, {
						key: "pool",
						value: function(e) {
							return this._client.pool(e)
						}
					}, {
						key: "getPoolNFTLeft",
						value: function(e) {
							return this._client.getPoolNFTLeft(e)
						}
					}, {
						key: "getUserStakeHisCnt",
						value: function(e, t) {
							return this._client.getUserStakeHisCnt(e, t)
						}
					}, {
						key: "getUserStakeHis",
						value: function(e, t, r) {
							return this._client.getUserStakeHis(e, t, r)
						}
					}, {
						key: "getAccount",
						value: function(e) {
							return this._client.getAccount(e)
						}
					}, {
						key: "formatUnits",
						value: function(e) {
							var t = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 18;
							return this._client.formatUnits(e, t)
						}
					}, {
						key: "parseEther",
						value: function(e) {
							return this._client.parseEther(e)
						}
					}, {
						key: "parseUnits",
						value: function(e) {
							var t = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 18;
							return this._client.parseUnits(e, t)
						}
					}]), w
				}()
		},
		avyV: function(f, v, o) {
			f.exports = {
				pc: "pc___1PYyO",
				title: "title___3Q114",
				tip: "tip___1wRGr",
				mobile: "mobile___2M967"
			}
		},
		ej9e: function(f, v, o) {
			f.exports = {
				modal: "modal___EpJsD",
				button: "button___2blkm"
			}
		},
		mNhR: function(f, v, o) {
			"use strict";
			var I = o("R+Pm"),
				L = Object(I.a)({
					scriptUrl: "//at.alicdn.com/t/font_2789311_1mm7wuj32fk.js"
				});
			v.a = L
		},
		r9ys: function(f, v) {
			f.exports = "data:image/png;base64,"
		},
		tTNn: function(f) {
			f.exports = JSON.parse('[{"anonymous":false,"inputs":[{"indexed":true,"internalType":"address","name":"owner","type":"address"},{"indexed":true,"internalType":"address","name":"spender","type":"address"},{"indexed":false,"internalType":"uint256","name":"value","type":"uint256"}],"name":"Approval","type":"event"},{"anonymous":false,"inputs":[{"indexed":true,"internalType":"address","name":"from","type":"address"},{"indexed":true,"internalType":"address","name":"to","type":"address"},{"indexed":false,"internalType":"uint256","name":"value","type":"uint256"}],"name":"Transfer","type":"event"},{"inputs":[{"internalType":"address","name":"owner","type":"address"},{"internalType":"address","name":"spender","type":"address"}],"name":"allowance","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"},{"inputs":[{"internalType":"address","name":"spender","type":"address"},{"internalType":"uint256","name":"amount","type":"uint256"}],"name":"approve","outputs":[{"internalType":"bool","name":"","type":"bool"}],"stateMutability":"nonpayable","type":"function"},{"inputs":[{"internalType":"address","name":"account","type":"address"}],"name":"balanceOf","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"},{"inputs":[],"name":"totalSupply","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"},{"inputs":[{"internalType":"address","name":"recipient","type":"address"},{"internalType":"uint256","name":"amount","type":"uint256"}],"name":"transfer","outputs":[{"internalType":"bool","name":"","type":"bool"}],"stateMutability":"nonpayable","type":"function"},{"inputs":[{"internalType":"address","name":"sender","type":"address"},{"internalType":"address","name":"recipient","type":"address"},{"internalType":"uint256","name":"amount","type":"uint256"}],"name":"transferFrom","outputs":[{"internalType":"bool","name":"","type":"bool"}],"stateMutability":"nonpayable","type":"function"}]')
		},
		wdOY: function(f, v) {
			f.exports = "data:image/png;base64,"
		}
	}]);

//# sourceMappingURL=p__Artwork~p__Artwork__Detail~p__Drop~p__Drop__Detail~p__MarketPlace~p__MarketPlace__Detail.6c0a3934.async.js.map